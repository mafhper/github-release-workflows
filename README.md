...
jobs:
  release:
    uses: mafhper/release-core/.github/workflows/release.yml@v1.1.0
    with:
...
O Core é tratado como uma API de automação. Consumidores fixam versões imutáveis (`@v1.1.0`); `@main` nunca é dependência permanente. Mudança incompatível no contrato gera `v2.0.0`. Tags publicadas não devem ser movidas.
